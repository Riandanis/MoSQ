#!/usr/bin/env python3
"""
Phase 3 + ssGSEA: IC50-Aware Pretraining with ssGSEA Pathway Scores.

Extends pretrain_phase3.py to include ssGSEA as a 4th cell-line Q-Former token.
The 5 KV tokens to Q-Former are: [Drug, Cancer, Tissue, RNA-BERT, ssGSEA]

Same DR-A objectives as Phase 3:
    A. IC50-Aware Supervised Contrastive Loss (IC50 quintile bins, T=0.1)
    C. Cross-Modal Reconstruction (mask drug OR cellline tokens)

Loss: 1.0 * L_contrastive + 0.5 * L_reconstruction

Loads from DR-A checkpoint (Phase 3), saves pretrained_phase3_ssgsea.pt.
"""
...
def load_pretrained_weights(model, checkpoint_path, device):
    ...
    # Filter: only load params that exist in both checkpoint and model with matching shape
    filtered = {}
    for k, v in state_dict.items():
        if k in model_dict and v.shape == model_dict[k].shape:
            filtered[k] = v

    # Special handling for ssGSEA projector - randomly initialize if not in checkpoint
    missing_in_ckpt = [k for k in model_dict if k not in filtered]
    if missing_in_ckpt:
        logger.info(f"Params NOT in checkpoint (ssGSEA or new): {missing_in_ckpt}")
...
def build_tokens_for_batch_with_ssgsea(model, batch, device):
    """
    Build the 5 KV tokens (drug + cancer + tissue + rna + ssgsea) for MultiToken + ssGSEA mode.

    Returns:
        tokens_list: list of 5 tensors, each [B, 1, D]
        combined: [B, 5, D] concatenated
    """
    ...
    ssgsea_embeds = batch.get('ssgsea_embed')
    if ssgsea_embeds is not None:
        ssgsea_embeds = ssgsea_embeds.to(device)
    ssgsea_available = batch.get('ssgsea_available')
    if ssgsea_available is not None:
        ssgsea_available = ssgsea_available.to(device)

    # Drug token
    drug_proj = model.projectors['drug'](drug_embeds).unsqueeze(1)  # [B, 1, D]
    drug_proj = drug_proj + model.modality_type_embeddings['drug']

    # Cell-line tokens (4 tokens: cancer, tissue, rna, ssgsea)
    ...
    cl_tokens[:, 3:4, :] = cl_tokens[:, 3:4, :] + model.modality_type_embeddings['ssgsea']

    cancer_tok = cl_tokens[:, 0:1, :]
    tissue_tok = cl_tokens[:, 1:2, :]
    rna_tok = cl_tokens[:, 2:3, :]
    ssgsea_tok = cl_tokens[:, 3:4, :]

    tokens_list = [drug_proj, cancer_tok, tissue_tok, rna_tok, ssgsea_tok]
    combined = torch.cat(tokens_list, dim=1)  # [B, 5, D]
...
def main():
    parser = argparse.ArgumentParser(description='Phase 3 + ssGSEA: IC50-Aware Pretraining')
    parser.add_argument('--checkpoint', type=str, default=DEFAULT_CHECKPOINT,
                        help='DR-A pretrained checkpoint path')
    parser.add_argument('--output_dir', type=str,
                        default=ROOT + 'checkpoints_save/checkpoints_phase3_ssgsea')
    ...
    parser.add_argument('--ssgsea_dim', type=int, default=768,
                        help='Dimension of ssGSEA embeddings')
    ...
    logger.info("=" * 60)
    logger.info("PHASE 3 + ssGSEA: IC50-Aware Pretraining with ssGSEA")
    logger.info("=" * 60)
    ...
    logger.info(f"ssGSEA dim: {args.ssgsea_dim}")
    ...
    model = ModalitySlotQFormerWithSsgsea(config, ssgsea_dim=args.ssgsea_dim).to(device)
    ...
    # Cell-line mask: replaces 4 cell-line tokens with 1 mask token
    ...
    logger.info(f"ssGSEA availability: {ic50_ds.cellline_has_ssgsea.sum().item()}/{ic50_ds.num_celllines} cell-lines")
    ...
    for epoch in range(args.epochs):
        ...
        pbar = tqdm(loader, desc=f"Phase 3+ssGSEA Epoch {epoch+1}/{args.epochs}")

        for batch in pbar:
            optimizer.zero_grad()

            # ---- Build tokens ----
            tokens_list, combined = build_tokens_for_batch_with_ssgsea(model, batch, device)
            # tokens_list: [drug, cancer, tissue, rna, ssgsea], each [B, 1, D]
            # combined: [B, 5, D]
            B = combined.size(0)
            ...
            if mask_drug:
                # Mask drug token, keep 4 CL tokens, reconstruct drug projected embedding
                ...
                # Mask 4 CL tokens with single mask, keep drug, reconstruct mean of CL embeddings
                cl_target = torch.cat([
                    tokens_list[1], tokens_list[2], tokens_list[3], tokens_list[4]
    ...
    # ---- Save checkpoint ----
    checkpoint_path = output_dir / 'pretrained_phase3_ssgsea.pt'
    checkpoint = {
        'epoch': args.epochs,
    ...
    history_path = output_dir / 'phase3_ssgsea_history.json'
    with open(history_path, 'w') as f:
        json.dump(history, f, indent=2)
    logger.info(f"Training history saved to {history_path}")

    logger.info("\n" + "=" * 60)
    logger.info("PHASE 3 + ssGSEA PRETRAINING COMPLETE")
    logger.info("=" * 60)
